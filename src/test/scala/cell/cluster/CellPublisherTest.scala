...
import ontologies.messages.MessageType.{Alarm, Handshake, Info, Init, Topology, Update}
...
class CellPublisherTest extends TestKit(ActorSystem("CellPublisherTest", CellPublisherTest.config))
    ...
    "A Cell Publisher" should {
        ...
        "after the parent response relative to the previous info request, sends a handshake message to the master" in {
