...
import system.ontologies.messages.MessageType.{Handshake, Init, Route, Topology, Update}
...
class MasterSubscriberTest extends TestKit(ActorSystem("SubscriberTest", MasterSubscriberTest.config))
    ...
    "A Subscriber of the system.master node" should {
        ...
        "Expect a Topology ACK from the TopologySupervisor in order to change its behaviour after " +
            "sending off all the stashed and incoming handshakes" in {
            
            tester ! MasterSubscriber.TopologyMappedACK
